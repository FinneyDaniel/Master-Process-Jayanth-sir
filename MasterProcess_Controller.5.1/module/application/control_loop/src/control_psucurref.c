...
void CONTROL_fnPSU_IRef(void)
{
    ...
    if (MATHConvtzRegs.AISensorPRT301 < 0)
    {
        MATHConvtzRegs.AISensorPRT301 = 0;
    }
    ...
    {
        if (STAT_tzStateMac.Present_st == OI_STAND_BY)
        {
            CANA_tzTxdRegs.tzPSUData.TotalCurrentSet = 0;
        }
        else if (STAT_tzStateMac.Present_st == OI_ARMED_POWER)
        {
            CANA_tzTxdRegs.tzPSUData.TotalCurrentSet = 0.0;

            CANA_tzTxdRegs.tzPSUData.CurrentSet =
                    CANA_tzTxdRegs.tzPSUData.TotalCurrentSet;
        }

        else if (STAT_tzStateMac.Present_st == OI_STATUS_CHECK)
        {
            CANA_tzTxdRegs.tzPSUData.TotalCurrentSet = 0.0;

            if (CANA_tzTxdRegs.tzPSUData.TotalCurrentSet
            ...
            CANA_tzTxdRegs.tzPSUData.CurrentSet =
                    CANA_tzTxdRegs.tzPSUData.TotalCurrentSet / ui32NodePresent;

        }

        else if (STAT_tzStateMac.Present_st == OI_STACK_POWER)
        {

            if ((CANB_tzSiteRxRegs.Start_H2Cmd == 1)
                    && (CANA_tzActNodeRegs_IO.bit.bt_node1 == 1)
                    && (CANA_tzActNodeRegs_IO.bit.bt_node2 == 1)
                    && (CANA_tzActNodeRegs_IO.bit.bt_node3 == 1)
                    && (CANA_tzActNodeRegs_IO.bit.bt_node4 == 1)
